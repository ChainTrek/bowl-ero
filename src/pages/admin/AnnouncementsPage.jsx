...
export default function AnnouncementsPage() {
    ...
    try {
      ...
      if (editingAnnouncementId) {
        ...
        setAnnouncements((prev) =>
          prev.map((item) =>
            item.id === editingAnnouncementId ? updatedAnnouncement : item
          )
        );
      ...
      setStatusMessage(
        `Error ${editingAnnouncementId ? 'updating' : 'adding'} announcement: ${error.message}`
      );
    } finally {
      setSubmitting(false);
    }
  ...
  async function handleToggleActive(item) {
    ...
    const confirmed = window.confirm(
      `Are you sure you want to ${actionLabel} "${item.title}"?`
    );
    ...
    try {
      ...
      setAnnouncements((prev) =>
        prev.map((entry) => (entry.id === item.id ? updatedAnnouncement : entry))
      );
        ...
        <form className="announcement-form" onSubmit={handleSubmit}>
          ...
          {editingAnnouncementId && (
            <p className="helper-text">
              Image replacement is not included yet. This edit mode updates the announcement details only.
            </p>
          )}
