...
export default function ReservationBlockoutsPage() {
	...
	async function handleSubmit(event) {
		...
		try {
			...
			if (editingBlockoutId) {
				...
				setSuccessMessage(
					'Reservation blockout updated successfully.',
				)
			} else {
				const newBlockout =
				...
				setSuccessMessage(
					'Reservation blockout added successfully.',
				)
			}
	...
	async function handleDelete(blockout) {
		const confirmed = window.confirm(
			`Delete "${blockout.title}" on ${formatReservationDate(
				blockout.block_date,
		...
		try {
			setDeletingBlockoutId(blockout.id)
			clearMessages()

			await deleteReservationBlockout(blockout.id)

			setBlockouts(previousBlockouts =>
				previousBlockouts.filter(
					item => item.id !== blockout.id,
				),
			)
			...
			setSuccessMessage(
				'Reservation blockout deleted successfully.',
			)
		} catch (error) {
			setErrorMessage(
							...
							return (
								<article
									className={`reservation-blockout-item ${
										!blockout.is_active
											? 'reservation-blockout-item--inactive'
											: ''
									}`}
									key={blockout.id}
								>
